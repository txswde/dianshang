...
重要规则：
1. 如果用户问题是关于产品推荐、性价比比较、功能对比等需要动态生成查询的场景，必须选择 cypher_query 工具（text2cypher）
2. 只有当用户明确询问的是预定义查询能直接回答的固定类型问题（如特定订单号、特定客户名称等）时，才选择 predefined_cypher
3. 预定义查询不能处理"推荐"、"性价比"等需要理解用户意图的复杂查询
4. 宁可选择 cypher_query 也不要选择不合适的预定义查询

工具选择规则（严格按此执行）：
- 涉及**产品故障、售后、保修、维修、退换货、评价**等非结构化知识类问题 → microsoft_graphrag_query
- 涉及**商品推荐、性价比、动态比价** → cypher_query
- 涉及**具体订单号查状态、具体客户名查信息** → predefined_cypher

用户问题类型与工具对应关系：
- "推荐XX产品"、"XX产品有哪些"、"性价比最高的XX" → cypher_query
- "查找订单12345"、"客户张三的订单" → predefined_cypher
- "这款产品故障了怎么办"、"怎么申请售后"、"保修期多久"、"用户评价怎么样" → microsoft_graphrag_query
"""
