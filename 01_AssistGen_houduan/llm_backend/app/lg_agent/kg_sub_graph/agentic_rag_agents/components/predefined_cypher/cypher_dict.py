...
predefined_cypher_dict: Dict[str, str] = {
    ...
    "smart_lighting": "MATCH (p:Product)-[:BELONGS_TO]->(c:Category) WHERE c.CategoryName = '智能灯具' RETURN p.ProductName, p.UnitPrice, p.UnitsInStock",
    "smart_air_purifier": "MATCH (p:Product)-[:BELONGS_TO]->(c:Category) WHERE c.CategoryName = '智能空气净化器' RETURN p.ProductName, p.UnitPrice, p.UnitsInStock",
    "smart_water_purifier": "MATCH (p:Product)-[:BELONGS_TO]->(c:Category) WHERE c.CategoryName = '智能净水器' RETURN p.ProductName, p.UnitPrice, p.UnitsInStock"
}