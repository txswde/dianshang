...
def create_tool_selection_node(
    ...
    async def tool_selection(
        ...
        """
        logger.info(f"[TOOL_SELECTION DEBUG] Received question: {state.get('question', '')}")
        # 调用工具选择链，生成针对每个任务要调用的工具名称和参数
        tool_selection_output: BaseModel = await tool_selection_chain.ainvoke(
            {"question": state.get("question", "")}
        )
        logger.info(f"[TOOL_SELECTION DEBUG] Tool selection output: {tool_selection_output}")
        logger.info(f"[TOOL_SELECTION DEBUG] Tool selection output type: {type(tool_selection_output)}")
        ...
        if tool_selection_output is not None:
            ...
            logger.info(f"[TOOL_SELECTION DEBUG] Selected tool: {tool_name}")
            logger.info(f"[TOOL_SELECTION DEBUG] Tool args: {tool_args}")
