    ...
    async def validate_cypher(state: CypherState) -> Dict[str, Any]:
        ...
        if not llm_validation:
            cypher_errors = validate_cypher_query_with_schema(
                graph=graph, cypher_statement=corrected_cypher
            )
            errors.extend(cypher_errors)

        # 自动修复常见的关系不存在问题：如果使用了不存在的关系，尝试使用节点属性替代
        if errors and any("不存在于模式中" in e or "does not exist" in e.lower() for e in errors):
            # 如果使用了 BELONGS_TO 关系但不存在，尝试替换为直接使用 CategoryName 属性
            if "BELONGS_TO" in corrected_cypher:
                import re
                # 替换 MATCH (p:Product)-[:BELONGS_TO]->(c:Category) WHERE c.CategoryName = 'X'
                # 为 MATCH (p:Product) WHERE p.CategoryName = 'X'
                fixed_cypher = re.sub(
                    r"MATCH \(p:Product\)-(\[:BELONGS_TO\])->\(c:Category\) WHERE c\.CategoryName = ['\"]([^'\"]+)['\"]",
                    r"MATCH (p:Product) WHERE p.CategoryName = '\2'",
                    corrected_cypher
                )
                if fixed_cypher != corrected_cypher:
                    logger.info(f"[CYPHER AUTO-FIX] Replaced BELONGS_TO relation with CategoryName property: {fixed_cypher}")
                    corrected_cypher = fixed_cypher
                    errors = []  # 清除之前的错误
