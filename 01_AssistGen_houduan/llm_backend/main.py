...
# 创建 FastAPI 应用实例
app = FastAPI(title="dianshang REST API")
    ...
    try:
        ...
        thread_config = {
            "configurable": {
                ...
                "file_path": None,  # file-query 类型时由客户端单独传递
            }
        }
...
@app.post("/api/langgraph/file_query")
async def langgraph_file_query(
    query: str = Form(...),
    user_id: int = Form(...),
    conversation_id: Optional[str] = Form(None),
    file: Optional[UploadFile] = File(None),
):
    """使用LangGraph处理文件查询，支持任意文件类型上传"""
    try:
        logger.info(f"Processing LangGraph file query for user {user_id}")
        ...
        try:
            if thread_id:
                state_history = list(graph.get_history(thread_id))
        except Exception:
            state_history = None
        ...
        logger.error(f"LangGraph file query error: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
